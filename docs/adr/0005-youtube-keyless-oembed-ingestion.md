...
- **The client detects playback failures.** Persistent `onError` codes `2`/`100`/`101`/`150` trigger the M3 error write; the server stores the GM-facing cause. The add-time result is only a hint and a 200 does not prevent a later errored flip. Recovery requires successful IFrame playback before clearing the stored error (issue #88).
