...
Within each layer, GMs organize sounds into sets. Sets are auto-populated by tags (with manual, hand-picked sets a future addition). Each set has configurable settings including loop, shuffle, and overlap rules for how multiple sets interact.
